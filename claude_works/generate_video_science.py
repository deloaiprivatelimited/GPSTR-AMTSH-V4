"""
generate_video_science.py
--------------------------
Science video pipeline: Chunks → Slides → Video → AWS S3 Upload → MongoDB Save → Local Cleanup

Same flow as maths pipeline but for science chapters.
No data.json — reads chapter/module metadata from _meta.json files.
...
CHUNKS_DIR  = Path("claude_works/chunks_science")
AUDIO_DIR   = Path("claude_works/audio_science")
OUTPUT_DIR  = Path("claude_works/videos_science")

ERROR_LOG   = Path("claude_works/science_pipeline_errors.log")
PROGRESS_LOG = Path("claude_works/science_pipeline_progress.log")
...
MONGO_COLLECTION = "science_videos"
...
# ==============================
# LOAD CHAPTER METADATA (from _meta.json files)
# ==============================

def load_chapter_metadata():
    """Build chapter_id -> chapter info lookup from _meta.json files in chunks_science."""
    if not CHUNKS_DIR.is_dir():
        return {}
    lookup = {}
    for ch_dir in CHUNKS_DIR.iterdir():
        if not ch_dir.is_dir():
            continue
        chapter_id = ch_dir.name
        # Parse class and chapter from dir name: std_10_chapter_1
        parts = chapter_id.split("_")
        cls = parts[1] if len(parts) >= 2 else ""
        ch_num = parts[-1] if len(parts) >= 4 else ""

        # Read first concept's _meta.json for chapter title and domain
        chapter_title = ""
        domain = ""
        ...
        lookup[chapter_id] = {
            ...
            "classes": [int(cls)] if cls.isdigit() else [],
            "chapters": [int(ch_num)] if ch_num.isdigit() else [],
        }
    return lookup
...
def post_process_chapter(merge_code, full_video_path, concept_videos, slides_dir, total_slides):
    ...
    for cv in concept_videos:
        ...
        s3_key    = f"science_videos/{merge_code}/{module_id}.mp4"
    ...
    full_s3_key = f"science_videos/{merge_code}/{merge_code}_full.mp4"
...
async def main():
    log_progress("\n========================================")
    log_progress("Science Video Pipeline -- Render + S3 + MongoDB")
    log_progress("========================================\n")
