"""
Multi-project parallel audio generation for science chunks.
- Only processes modules where ALL chunks completed without errors
...
- Polls for new chunks from chunks_science/
- Splits modules across N Google Cloud projects

Usage:
  python claude_works/generate_audio_science.py
"""
import json
import wave
import hashlib
        ...
        for attempt in range(retries + 1):
            try:
                ...
                if "429" in err or "RESOURCE_EXHAUSTED" in err:
                    ...
                    print(f"    [W{self.worker_id}] 403 error: {err[:100]}")
                    return False
                else:
                    if attempt < retries:
                        wait = 15 * (attempt + 1)
                        ...
                        time.sleep(wait)
                        continue
                    print(f"    [W{self.worker_id}] TTS FAILED: {err[:150]}")
                    return False
        return False
...
# PROCESS ONE MODULE
# ==========================================
def process_module(chapter_code, module_id, worker):
    chunk_dir       = CHUNKS_FOLDER / chapter_code / module_id
    meta_path       = chunk_dir / "_meta.json"
    audio_dir       = AUDIO_FOLDER / chapter_code / module_id
    ...
    # ONLY process if chunks have ZERO errors
    if meta.get("errors"):
        print(f"  [W{worker.worker_id}] SKIP {module_id}: chunk generation had {len(meta['errors'])} errors")
        return "chunk_errors"
    ...
    # ---- Step 2: Figure out which need TTS ----
    chunks_needing_tts = []
    chunks_already_done = []
        ...
        for future in as_completed(futures):
            try:
                ...
                if result.get("status") == "failed":
                    print(f"    [W{worker.worker_id}] FAILED TTS: {fn} — {result.get('error', 'unknown')}")
            except Exception as e:
                fn = futures[future]
                chunk_results[fn] = {
                    "status": "failed",
                ...
                print(f"    [W{worker.worker_id}] FAILED TTS: {fn} — {e}")

    # ---- Step 4: Save _audio_meta.json ----
...
def get_ready_modules():
    """Find modules that have chunks with ZERO errors and need audio."""
    ready = []
    ...
    for mc in sorted(CHUNKS_FOLDER.iterdir()):
        ...
        for mod_dir in sorted(mc.iterdir()):
            ...
            # ONLY pick modules with zero chunk errors
            try:
                meta = json.loads(meta_path.read_text(encoding="utf-8"))
                if meta.get("errors"):
                    continue
                if not meta.get("chunk_order"):
                    continue
            except Exception:
                continue
            ...
            if not audio_meta_path.exists():
                ready.append((chapter_code, module_id))
                continue

            # Has audio meta — check for failures to retry
            try:
...
def generate_summary():
    print("\n" + "=" * 60)
    print("SCIENCE AUDIO GENERATION SUMMARY")
    print("=" * 60)
...
def main():
    ...
    print(f"{'='*60}")
    print(f"SCIENCE AUDIO GENERATION — MULTI-PROJECT")
    print(f"{'='*60}")
    print(f"  Model:           {MODEL}")
    print(f"  Speaking rate:   {SPEAKING_RATE}")
    print(f"  Chunks from:     {CHUNKS_FOLDER}")
    print(f"  Audio to:        {AUDIO_FOLDER}")
    ...
    while True:
        ...
        if not ready:
            ...
            if empty_polls >= 60:
                ...
                break
            print(f"No modules ready. Waiting {POLL_INTERVAL}s... ({empty_polls}/60)")
